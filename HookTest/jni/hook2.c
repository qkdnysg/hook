...
{   int i, j;
    char temp;
    for(i = 0, j = strlen(str) - 2;//-1得到的是结束字符，-2得到的是最后一个字符
        i <= j; ++i, --j) {
        temp = str[i];
        str[i] = str[j];
        str[j] = temp;
...
{
    char* str;
    str = (char *)calloc((strlen+1) * sizeof(char), 1);//申请空间并清零
...
void hookSysCallBefore(pid_t pid)
{
    ...
    if(sysCallNo == __NR_write)
    {
        ...
        modifyString(pid, regs.ARM_r1, regs.ARM_r2);//传递参数：字符串地址（r1）,字符串长度（r2）
    }
}
